...
public class TestProjectSensorContextExtra {
  ...
  public static SensorContextTester createContext() throws IOException {
    ...
    context.fileSystem().add(
        new TestInputFileBuilder(BASEDIR, FILE6).setLanguage(Constants.LANGUAGE_KEY).setType(
            Type.MAIN).setCharset(Charset.defaultCharset()).setContents(Files.toString(new File(BASEDIR, FILE6), Charset.defaultCharset())).build());
    context.fileSystem().add(
