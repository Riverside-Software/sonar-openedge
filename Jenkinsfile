      ...
      steps {
        ...
        script {
          withEnv(["PATH+MAVEN=${tool name: 'Maven 3', type: 'hudson.tasks.Maven$MavenInstallation'}/bin"]) {
            if (("master" == env.BRANCH_NAME) || ("develop" == env.BRANCH_NAME) || env.BRANCH_NAME.startsWith("release") || env.BRANCH_NAME.startsWith("hotfix") || env.BRANCH_NAME.startsWith("feature/ANTLR2-deprecation")) {
    ...
    stage ('SonarQube analysis') {
      steps {
        script {
	      ...
	      } else if (env.BRANCH_NAME.startsWith("feature/ANTLR2-deprecation")) {
	        sh "mvn -Dsonar.host.url=http://sonar.riverside-software.fr -Dsonar.login=${env.SQ_TOKEN} -Dsonar.branch.name=antlr4 sonar:sonar"
