...
public class BuiltinClasses {
  ...
  private static Collection<ITypeInfo> readClasses(@Nonnull OpenEdgeVersion version) {
    ...
    try (var input = BuiltinClasses.class.getClassLoader().getResourceAsStream(version.getClassPath());
      ...
      for (var clz : clzArray.builtinClasses) {
        ...
        if (clz.enums != null) {
          ...
          for (var enumEntry : clz.enums) {
            var prop = new PropertyElement(enumEntry.name, true, new DataType(clz.name));
