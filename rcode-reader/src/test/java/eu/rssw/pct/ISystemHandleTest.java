...
public class ISystemHandleTest {
  ...
  public void test02() {
    for (OpenEdgeVersion version : OpenEdgeVersion.values()) {
      ...
      assertNotNull(syshdl);
      var list = syshdl.getAllAttributes();
      assertEquals(list.size(), 23);
      var list2 = syshdl.getAllMethods();
  ...
  public void test03() {
    for (OpenEdgeVersion version : OpenEdgeVersion.values()) {
      ...
      var list2 = syshdl.getAllAttributes();
      assertEquals(list2.size(), 4);
      assertEquals(list2.get(0).getName(), "HANDLE");
      assertTrue(list2.get(0).isReadOnly());
      assertFalse(list2.get(0).isWriteOnly());
