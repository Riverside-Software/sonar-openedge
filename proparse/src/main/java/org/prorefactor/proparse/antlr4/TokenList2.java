 ...
 * ID.
 */
public class TokenList2 implements TokenSource {
  private final TokenSource source;
  private final Deque<ProToken> heap = new LinkedList<>();

  private int currentPosition;
  private ProToken currentToken;

  public TokenList2(TokenSource input) {
    this.source = input;
  }
  ...
  private void reviewObjcolon() {
    ...
    while (true) {
      ...
      if ((heap.peekLast() != null) && (heap.peekLast().getType() == NodeTypes.NAMEDOT)) {
        clsName.addFirst(tok);
        clsName.addFirst(heap.pollLast());
        tok = heap.removeLast();
